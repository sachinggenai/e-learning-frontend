...
describe('service contract smoke tests', () => {
  ...
  it('calls canonical scoring calculate endpoint', async () => {
    mockedHttp.post.mockResolvedValueOnce({ data: { totalScore: 80, componentResults: [] } } as any);
    ...
    expect(mockedHttp.post).toHaveBeenCalledWith('/courses/course-1/scoring/calculate', {
      answers: [
        {
          componentId: 'cmp-1',
          componentType: 'mcq',
  ...
  it('calls canonical page completion endpoint', async () => {
    ...
    expect(mockedHttp.post).toHaveBeenCalledWith('/courses/course-1/pages/page-1/completion', {
      componentStates: [
        {
          componentId: 'cmp-1',
          completed: true,
  ...
  it('allows open interactionType values in interaction logging', async () => {
    ...
    expect(mockedHttp.post).toHaveBeenCalledWith('/courses/course-1/interactions', {
      pageId: 'page-1',
      componentId: 'cmp-1',
      interactionType: 'reveal',
      learnerId: 'learner-1',
    ...
    mockedHttp.post
      ...
      .mockResolvedValueOnce({
        ...
        headers: {
          ...
          'content-disposition': 'attachment; filename="course-1_scorm_1_2.zip"',
        },
      } as any);
