...
export const CircularProgress: React.FC<CircularProgressProps> = React.memo(({
  ...
  const resolvedFill = fillColor ?? (clamped >= 80 ? '#22c55e' : clamped >= 50 ? '#f59e0b' : '#ef4444');
  ...
  return (
    ...
    >
      ...
      {showLabel && (
        <span className="circular-progress__label" style={{ color: resolvedFill }}>
          {Math.round(clamped)}%
        </span>
      )}
    </div>
  ...
  return (
    ...
    >
      <div className="quiz-feedback__header">
        <span className="quiz-feedback__icon">
          {isCorrect ? '✓' : '✗'}
        </span>
        ...
        <div className="quiz-feedback__answers">
          {userAnswer && (
            <div className="quiz-feedback__answer-row">
              <span className="quiz-feedback__answer-label">Your answer:</span>
              <span className="quiz-feedback__answer-value">{userAnswer}</span>
            </div>
          )}
          {!isCorrect && correctAnswer && (
            <div className="quiz-feedback__answer-row">
              <span className="quiz-feedback__answer-label">Correct answer:</span>
...
export const ScoreSummary: React.FC<ScoreSummaryProps> = React.memo(({
  ...
  return (
    <div className={`score-summary ${passed ? 'score-summary--pass' : 'score-summary--fail'}`}>
      ...
      <div className="score-summary__main">
        ...
        <div className="score-summary__details">
          ...
          <span className={`score-summary__status ${passed ? 'score-summary__status--pass' : 'score-summary__status--fail'}`}>
