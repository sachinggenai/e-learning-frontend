...
const createMockStore = (initialState?: any) => {
  ...
  const store = configureStore({
    reducer: {
      editor: (state = defaultState.editor) => state,
      course: (state = defaultState.course) => state,
...
describe("PageManager - Inline Title Editing", () => {
  ...
  it.skip("saves title on Enter key and calls updatePageTitleThunk", async () => {
  ...
  it.skip("saves title on blur (clicking outside)", async () => {
    const { updatePageTitleThunk } = require("../store/slices/courseSlice");
  ...
  it.skip("trims whitespace from title before saving", async () => {
    const { updatePageTitleThunk } = require("../store/slices/courseSlice");
  ...
  it.skip("shows saving indicator while saving", async () => {
    render(
      <Provider store={store}>
        <PageManager />
      </Provider>,
  ...
  it("allows editing different pages independently", () => {
    ...
    // Cancel edit
    fireEvent.keyDown(screen.getByDisplayValue("Introduction"), {
      key: "Escape",
    });
