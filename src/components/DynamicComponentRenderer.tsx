...
function normaliseComponent(
  c: Component & Record<string, any>,
): { id: string; typeId: string; data: Record<string, any> } & Component {
  return {
    ...c,
...
> {
  ...
  componentDidCatch(error: Error, info: React.ErrorInfo): void {
    console.error(
      `[DynamicComponentRenderer] Error in component ${this.props.componentId}:`,
      error,
      info,
    );
  }
        ...
        <div className="dcr-error" role="alert">
          ...
          <p>
            {this.state.error?.message ??
              "Unknown error rendering this component."}
          </p>
          <button
...
export const DynamicComponentRenderer: React.FC<DynamicComponentRendererProps> =
  React.memo((props) => {
    ...
    const component = useMemo(
      () => normaliseComponent(rawComponent),
      [rawComponent],
    );
    ...
    const handleChange = useCallback(
      (update: Partial<ComponentDataUpdate>) => {
        if (mode === "edit") {
