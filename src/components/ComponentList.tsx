...
import type { ComponentDefinition, ComponentDataUpdate, ComponentInteractionEvent } from '../types/registry';
import {
  selectComponent,
...
export const ComponentList: React.FC<ComponentListProps> = React.memo(({
  ...
  const handleChange = useCallback((componentId: string, update: Partial<ComponentDataUpdate>) => {
    const dataUpdate = update.data ?? {};
    // Optimistic local update for instant feedback
    dispatch(updateComponentLocal({
      pageId,
      componentId,
  ...
  const handleDuplicate = useCallback((componentId: string) => {
    ...
    dispatch(duplicateComponentLocal({ pageId, componentId, newComponentId: newId }));
