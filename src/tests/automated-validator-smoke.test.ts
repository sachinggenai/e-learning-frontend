...
describe("Smoke Tests: Component Validation (Automated)", () => {
  ...
  test("SMOKE-02: Valid welcome component passes validation", async () => {
    ...
    const welcomeErrors = result.errors.filter(
      (e) =>
        e.field?.includes("pages[0]") ||
        e.message.toLowerCase().includes("page 1"),
    );
  ...
  test("SMOKE-03: Invalid MCQ component is caught by validation", async () => {
    ...
    // CRITICAL: await the async validate() method
    const result: ValidationResult = await validator.validate(
      invalidCourse as any,
    );

    const mcqErrors = result.errors.filter(
      (e) =>
    ...
    mcqErrors
      .slice(0, 3)
      .forEach((err) => console.log(`     - ${err.message}`));
  ...
  test("SMOKE-05: True/False question validation", async () => {
    ...
    for (const test of tests) {
      ...
      if (test.shouldError) {
        ...
        console.log(
          `  ✅ "${test.name}" validation result: ${hasErrors ? "errors found" : "passed"}`,
        );
      }
    }
  ...
  test("SMOKE-07: Matching question validation", async () => {
    ...
    // CRITICAL: await the async validate() method
    const validResult: ValidationResult = await validator.validate(
      validMatching as any,
    );
    const invalidResult: ValidationResult = await validator.validate(
      invalidMatching as any,
    );
    ...
    console.log(
      `  ✅ Invalid matching: ${invalidResult.errors.length} errors caught`,
    );
  ...
  test("SMOKE-08: Summary test with all validators", async () => {
    ...
    console.log(
      "🎉 SMOKE-08: COMPLETE - Comprehensive validation test passed\n",
    );
  });
});
