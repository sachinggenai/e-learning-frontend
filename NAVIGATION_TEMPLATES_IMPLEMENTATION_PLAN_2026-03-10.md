...
### 🔴 Critical Issue 6: Default Data Shape Migration Gaps

**Problem:** Current defaults in `src/data/componentRegistryData.ts` do not match planned interfaces for new templates.

**Current Mismatches Found:**
- `module-overview` currently uses `objectives: ['Objective 1']` (string array), but planned interface requires object array with IDs.
- `summary-takeaways` currently uses `keyPoints: ['Key point 1', 'Key point 2']` (string array), but planned interface requires object array with IDs/emphasis.
- `learning-roadmap` default data is minimal and missing `status` values required by planned milestone schema.

**Action Items:**
- [ ] Add a `defaultData migration` subtask for all 3 new templates before editor wiring
- [ ] Ensure migrated defaults are backward-safe with fallback parsing in preview/editor
- [ ] Add tests for legacy shape compatibility (string arrays -> object arrays)

---

### 🟡 Important Issue 7: Duplicate Design Doc Content Cleanup

**Problem:** `DESIGN_PLAN_RESOURCES_DOWNLOADS.md` contains duplicated content block near the end.

**Action Items:**
- [ ] Remove duplicate section from `DESIGN_PLAN_RESOURCES_DOWNLOADS.md` before implementation starts
- [ ] Re-check test counts and section numbering after cleanup
...
- [ ] Add normalization for legacy objective string arrays during transition
...
- [ ] Add fallback status normalization for legacy milestone entries missing `status`
...
- [ ] Add normalization for legacy key point string arrays during transition
...
- [ ] Run: `CI=true npm test -- --watchAll=false` (all tests)
...
**Plan Version:** 1.1  
**Last Updated:** March 10, 2026 (re-verified)  
